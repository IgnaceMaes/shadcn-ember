...
const CLIENTS = [
  {
    ...
    config: {
      mcpServers: {
        shadcnEmber: {
          command: 'npx',
          args: [`shadcn-ember@${SHADCN_MCP_VERSION}`, 'mcp'],
        },
      },
    },
  },
  ...
  },
  {
    ...
    config: `[mcp_servers.shadcn_ember]
command = "npx"
args = ["shadcn-ember@${SHADCN_MCP_VERSION}", "mcp"]
`,
  },
    ...
    config: {
      ...
      mcp: {
        shadcnEmber: {
          type: 'local',
          enabled: true,
          command: ['npx', `shadcn-ember@${SHADCN_MCP_VERSION}`, 'mcp'],
        },
      },
    },
  },
...
const DEPENDENCIES = [`shadcn-ember@${SHADCN_MCP_VERSION}`]
  ...
  .action(async (opts, command) => {
    try {
      ...
      if (options.client === 'codex') {
        ...
        logger.break()
        logger.log('To configure the shadcn-ember MCP server in Codex:')
        logger.break()
        logger.log(
        ...
        logger.log()
        logger.info(`[mcp_servers.shadcn_ember]
command = "npx"
args = ["shadcn-ember@${SHADCN_MCP_VERSION}", "mcp"]`)
        logger.break()
