  ...
  return {
    tools: [
      ...
      },
      {
        ...
        description:
          'Get the shadcn-ember CLI add command for specific items in a registry. This is useful for adding one or more components to your project.',
        inputSchema: zodToJsonSchema(
          z.object({
            items: z
              .array(z.string())
          ...
          content: [
            {
              ...
              text: dedent`The following registries are configured in the current project:
                ...
                \`${await npxShadcnEmber('view @name-of-registry')}\`

                For example: \`${await npxShadcnEmber(
                  'view @shadcn',
                )}\` or \`${await npxShadcnEmber(
                  'view @shadcn @acme',
      ...
      case 'get_add_command_for_items': {
        ...
        return {
          content: [
            {
              type: 'text',
              text: await npxShadcnEmber(`add ${args.items.join(' ')}`),
            },
          ],
        }
      }
