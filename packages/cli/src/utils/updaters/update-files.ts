...
export function resolvePageTarget(
  ...
  // Ember uses standard paths, no transformation needed
  return target
...
// Resolve imports using Ember-specific tooling
async function resolveImports(filePaths: string[], config: Config) {
  ...
  for (const filepath of filePaths) {
    ...
    const isGtsFile = filepath.endsWith('.gts') || filepath.endsWith('.gjs')

    try {
      let newCode = content

      // Transform function that resolves imports
      const transformImports = (code: string): string => {
        // Use ts-morph for import resolution
      ...
      // For .gts/.gjs files, use codemod-utils to preserve <template> tags
      if (isGtsFile) {
        newCode = updateJavaScript(content, transformImports)
      }
      else {
        // For regular .ts/.js files, transform directly
        newCode = transformImports(content)
      }
...
export function resolveModuleByProbablePath(
  ...
  files: string[],
  config: Config,
  extensions: string[] = ['.gts', '.gjs', '.hbs', '.ts', '.js', '.tsx', '.jsx', '.css'],
) {
...
export function toAliasedImport(
  ...
  const codeExts = ['.gts', '.gjs', '.ts', '.tsx', '.js', '.jsx']
