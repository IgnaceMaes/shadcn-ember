...
it('transform tailwind prefix', async () => {
  expect(
    await transform({
      ...
      export type { ButtonType } from "./Button.gts"
  ...
  expect(
    await transform({
      ...
      raw: `<template>
      <div
        id="testing" ...attributes {{on "click" this.handleSomething}} data-test={{true}}
        class={{cn
          "mt-4"
          "bg-background hover:bg-muted"
          "text-primary-foreground sm:focus:text-accent-foreground"
          "flex"
          @class
        }}
      >
