...
```bash
npx shadcn-ember@latest mcp init --client claude
```
...
```bash
npx shadcn-ember@latest mcp init --client cursor
```
...
```bash
npx shadcn-ember@latest mcp init --client vscode
```
...
> [!WARNING]
> **Note:** The `shadcn-ember` CLI cannot automatically update `~/.codex/config.toml`. You'll need to add the configuration manually for Codex.

**Run the following command** in your project:

```bash
npx shadcn-ember@latest mcp init --client codex
```
...
> [!WARNING]
> **Note:** The `shadcn-ember` CLI cannot automatically update `~/.codex/config.toml`. You'll need to add the configuration manually.
...
```json title="opencode.json" showLineNumbers
{
    ...
    "mcp": {
        "shadcnEmber": {
            "type": "local",
            "enabled": true,
            "command": ["npx", "shadcn-ember@latest", "mcp"]
        }
    }
}
```
...
3. **Verify Installation** - Ensure `shadcn-ember` is installed in your project
